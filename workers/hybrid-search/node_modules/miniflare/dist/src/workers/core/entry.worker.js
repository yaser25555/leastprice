  ...
  ArrayBufferView(value) {
    if (ArrayBuffer.isView(value)) {
      ...
      if (!ALLOWED_ARRAY_BUFFER_VIEW_CONSTRUCTORS.some((c) => c.name === name)) {
        for (let ctor of ALLOWED_ARRAY_BUFFER_VIEW_CONSTRUCTORS)
          if (value instanceof ctor) {
            name = ctor.name;
            break;
          }
      }
      let buf = value.buffer, off = value.byteOffset;
      return (off !== 0 || buf.byteLength !== value.byteLength) && (buf = buf.slice(off, off + value.byteLength), off = 0), [name, buf, off, value.byteLength];
    }
  },
  RegExp(value) {
  ...
  async #fetch(request) {
    ...
    if (opHeader === ProxyOps.GET) {
      ...
      if (assert3(typeof func == "function"), isFetcherFetch(targetName, keyHeader)) {
        let originalUrl = request.headers.get(CoreHeaders.OP_ORIGINAL_URL) ?? request.headers.get(CoreHeaders.ORIGINAL_URL), url = new URL(originalUrl ?? request.url);
        return request = new Request(url, request), request.headers.delete(CoreHeaders.OP_SECRET), request.headers.delete(CoreHeaders.OP), request.headers.delete(CoreHeaders.OP_TARGET), request.headers.delete(CoreHeaders.OP_KEY), request.headers.delete(CoreHeaders.OP_ORIGINAL_URL), request.headers.delete(CoreHeaders.ORIGINAL_URL), request.headers.delete(CoreHeaders.DISABLE_PRETTY_ERROR), func.call(target, request);
      }
      let args;
...
function validateCdnCgiRequest(request, routes, upstreamUrl) {
  if (!isCdnCgiRequest(request.url) && !isCdnCgiRequest(request.headers.get(CoreHeaders.ORIGINAL_URL)))
    return;
...
function maybeInjectLiveReload(response, env, ctx) {
  ...
  if (liveReloadScript === void 0 || !response.headers.get("Content-Type")?.toLowerCase().includes("text/html"))
    return response;
  let headers = new Headers(response.headers), contentLength = parseInt(headers.get("content-length") ?? "NaN");
...
var entry_worker_default = {
  async fetch(request, env, ctx) {
    ...
    if (requestUrl.pathname === CorePaths.PLATFORM_PROXY)
      return request.headers.get(CoreHeaders.OP) !== null ? handleProxy(request, env) : new Response("Invalid proxy request", { status: 400 });
    ...
    try {
      if (env[CoreBindings.SERVICE_LOCAL_EXPLORER] && (url.pathname === CorePaths.EXPLORER || url.pathname.startsWith(`${CorePaths.EXPLORER}/`)))
        return await env[CoreBindings.SERVICE_LOCAL_EXPLORER].fetch(request);
      if (env[CoreBindings.TRIGGER_HANDLERS]) {
        if (url.pathname === CorePaths.SCHEDULED || /* legacy URL path */
        url.pathname === CorePaths.LEGACY_SCHEDULED)
          return url.pathname === CorePaths.LEGACY_SCHEDULED && ctx.waitUntil(
            env[CoreBindings.SERVICE_LOOPBACK].fetch(
              "http://localhost/core/log",
              {
                method: "POST",
      ...
      let streamService = env[CoreBindings.SERVICE_STREAM];
      if ((url.pathname === CorePaths.STREAM_VIDEO || url.pathname.startsWith(`${CorePaths.STREAM_VIDEO}/`)) && streamService)
