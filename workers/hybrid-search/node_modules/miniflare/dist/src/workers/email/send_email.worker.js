...
function synthesizeMessageId(senderEmail) {
  let alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789", bytes = crypto.getRandomValues(new Uint8Array(36)), id = Array.from(bytes, (b) => alphabet[b % alphabet.length]).join(""), domain = senderEmail.slice(senderEmail.lastIndexOf("@") + 1);
  return `<${id}@${domain}>`;
}
...
var SendEmailBinding = class extends WorkerEntrypoint {
  ...
  async storeTempFile(content, extension, prefix) {
    ...
    let fileName = `${crypto.randomUUID()}.${extension}`, url = new URL(`${prefix}/${fileName}`, "http://placeholder/");
    return await this.env.MINIFLARE_EMAIL_DISK.fetch(url, {
      method: "PUT",
      body
    }), joinPath(this.env.email_directory, prefix, fileName);
  }
  ...
  ${file}`
      ), { messageId: synthesizeMessageId(emailMessage.from) };
    } else {
